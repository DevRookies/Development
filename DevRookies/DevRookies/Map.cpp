...
bool Map::Load(const char* file_name)
{
	...
	// Load layer info ----------------------------------------------
	pugi::xml_node maplayer_node;
	for (maplayer_node = map_file.child("map").child("layer"); maplayer_node && ret; maplayer_node = maplayer_node.next_sibling("layer"))
	{
		MapLayer* maplayer = new MapLayer();

		if (ret == true)
		{
			ret = LoadLayer(maplayer_node, maplayer);
		}
...
bool Map::LoadTilesetImage(pugi::xml_node& tileset_node, TileSet* set)
{
	...
	{
		set->texture = App->textures->Load(PATH(folder.GetString(), image.attribute("source").as_string()));
		int w, h;
